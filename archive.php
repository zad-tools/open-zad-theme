<?php
/**
 * The template for displaying archive pages (category, tag, author, date, CPT).
 *
	...
	<main id="main" class="site-main <?php echo is_active_sidebar( 'sidebar-1' ) ? 'lg:col-span-8' : 'lg:col-span-12'; ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			<header class="page-header mb-8 border-b border-border pb-6">
				<?php
				the_archive_title( '<h1 class="page-title text-2xl md:text-3xl font-black">', '</h1>' );
				the_archive_description( '<div class="archive-description text-text-muted mt-3">', '</div>' );
				?>
			</header>
